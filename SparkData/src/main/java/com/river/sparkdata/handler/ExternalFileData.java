...
public class ExternalFileData {
	 ...
	 */
	public ExternalFileData(JavaSparkContext sparkContext) {
	...
	public void callFileData(String filePath) {
		...
		System.out.println("(ExternalFileData)Data = " + carData.collect());

		/* Printing first n=3 records in carData */
		System.out.println("(ExternalFileData)First Three Records = " + carData.take(3));

		/* Printing first record in carData */
		System.out.println("(ExternalFileData)First Record = " + carData.first());

		/* Printing the count of records in carData */
		System.out.println("(ExternalFileData)Count = " + carData.count());

		/* Printing each record in carData */
		carData.collect().forEach(x -> System.out.println("(ExternalFileData)" + x));
